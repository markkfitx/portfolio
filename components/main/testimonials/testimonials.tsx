...
function TestimonialCard({ item }: { item: Testimonial }) {
  return (
    <div className="flex h-full flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6 transition-colors hover:border-white/15 hover:bg-white/[0.05]">
      <blockquote className="flex-1 text-sm leading-relaxed text-white/75 md:text-[0.9375rem]">
...
export default async function Testimonials({ id, className }: TestimonialsProps) {
  ...
  return (
    <Section
      id={id}
      className={clsx(
        "section-shell z-10 bg-linear-to-t from-neutral-900 to-black py-24 md:py-32 lg:py-36",
        className
      )}
    >
      <SectionHeader
