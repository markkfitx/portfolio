...
export default function NavItem({label,url,idx, children}:navlink){
    ...
    return(
       <Link href={href} className="flex flex-row items-end content-center bg-transparent text-white hover:text-gray-300 font-semibold py-1 px-3 text-sm">
            {label}
            <small className="text-[10px]"><span className="mx-2">/</span>0{idx+1}</small>
        </Link>
    );
}